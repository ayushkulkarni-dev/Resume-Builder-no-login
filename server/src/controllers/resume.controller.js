...
export const getResumes = async (req, res, next) => {
  try {
    const resumes = await resumeService.getResumesByUser();
...
export const updateSection = async (req, res, next) => {
  try {
    ...
    const resume = await resumeService.updateSection(req.params.id, section, req.body.data);
    return res.json({ success: true, data: resume });
  } catch (error) {
    if (error.statusCode) return res.status(error.statusCode).json({ success: false, message: error.message });
    next(error);
...
export const updateTemplate = async (req, res, next) => {
  try {
    ...
    const resume = await resumeService.updateTemplate(req.params.id, templateId);
    return res.json({ success: true, data: resume });
  } catch (error) {
    if (error.statusCode) return res.status(error.statusCode).json({ success: false, message: error.message });
    next(error);
...
export const uploadResume = async (req, res, next) => {
  try {
    ...
    const resume = await resumeService.createFromUpload(parsedSections);
